# Setup do ambiente dos hands-on — Redes Neurais e Deep Learning (aulas 01 a 08)
# Uso:  powershell -ExecutionPolicy Bypass -File setup\instalar.ps1
#
# Cria o venv em <raiz do curso>\.venv, instala torch (CUDA se houver GPU NVIDIA)
# e as demais dependências do requirements.txt.

$ErrorActionPreference = "Stop"
$raiz = Split-Path -Parent $PSScriptRoot   # pasta raiz do curso (onde está o requirements.txt)
...
& $py -m ipykernel install --prefix $venv --name python3 --display-name "Python (Redes Neurais)"
...
Write-Host "Para abrir os notebooks (com o ambiente ativado):"
